...
export class RenameNodeCommand implements ICommand {
  ...
  execute = async () => {
    ...
    const result = await hooks.renameNode.call(args, async (a) => {
      // @ts-ignore
      const { nodeConfig, graphMeta, commandService, modelService, updateNodeNameService } = a;
      const preNodeName = nodeConfig.label;

      const getAppContext: () => {
        graphMeta: any;
        modelService: IModelService | undefined;
